...
def parse_body():
    """Parse request body — handles both JSON object and JSON string."""
    raw = request.get_data(as_text=True)
    try:
        data = json.loads(raw)
        # If Flask parsed a JSON string (double-encoded), parse again
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'service': 'Unrigged Merge v11'})
...
def debug():
    ...
    return jsonify({
        ...
        'parsed_is_str': isinstance(parsed, str)
    })
...
def _assemble_worker(job_id, data):
    try:
        if not data or not isinstance(data, dict):
            jobs[job_id] = {'status': 'error', 'error': f'data is not dict: {type(data).__name__} = {str(data)[:100]}'}
            return
        ...
        if not clip_urls:
            jobs[job_id] = {'status': 'error', 'error': 'Inga clip_urls'}
            return
        if not audio_url:
            jobs[job_id] = {'status': 'error', 'error': 'Ingen audio_url'}
            return
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            jobs[job_id]['progress'] = 'Laddar upp...'
